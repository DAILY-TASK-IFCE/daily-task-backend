...
class InviteList(ResourceModel):
    ...
    def post(self, new_invite_data):
        if Invite.query.filter_by(
            user_id=new_invite_data["user_id"], team_id=new_invite_data["team_id"]
        ).first():
            return {
                "message": "Já existe um convite desse time para esse usuário."
            }, 409
...
class AcceptInviteId(ResourceModel):
    ...
    def delete(self, id):
        ...
        new_team_user = TeamUser(
            ...
            type_id=os.getenv("TEAM_REGULAR_ID"),
        )
