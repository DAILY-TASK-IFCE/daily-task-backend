...
class DailyList(ResourceModel):
    ...
    def get(self, args):
        ...
        if team_id:
            query = query.join(TeamUser).filter(
                TeamUser.team_id == team_id, Daily.team_user_id == TeamUser.id
            )
    ...
    def post(self, new_daily_data):
        ...
        if Daily.query.filter_by(
            date=new_daily_data["date"], team_user_id=new_daily_data["team_user_id"]
        ).first():
